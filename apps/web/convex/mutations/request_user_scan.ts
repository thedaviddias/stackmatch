...
const submittedRepoValidator = v.object({
  owner: v.string(),
  name: v.string(),
...
const requestUserScanArgs = {
  repos: v.array(submittedRepoValidator),
  apiKey: v.string(),
  ownerProfile: v.optional(ownerProfileValidator),
  submitter: v.optional(submitterValidator),
};
...
async function requestUserScanHandler(
  ctx: MutationCtx,
  args: {
    repos: Array<{
      owner: string;
  ...
  if (args.submitter && owner && fullNames.length > 0) {
    await ctx.db.insert("scanSubmissions", {
      ...
      ...(args.submitter.githubLogin ? { submittedByGitHubLogin: args.submitter.githubLogin } : {}),
...
export const requestUserScan = mutation({
  args: requestUserScanArgs,
  handler: requestUserScanHandler,
});

export const requestUserScanInternal = internalMutation({
  args: requestUserScanArgs,
  handler: requestUserScanHandler,
});

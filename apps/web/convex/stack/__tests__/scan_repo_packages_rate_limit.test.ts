...
import { scanRepoPackages } from "../scan_repo_packages";
...
describe("scanRepoPackages GitHub failures", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  it("schedules a retry when the tree scan is rate-limited", async () => {
    ...
    await getHandler(scanRepoPackages)(ctx, {
      repoId: REPO_ID,
      owner: OWNER,
      name: NAME,

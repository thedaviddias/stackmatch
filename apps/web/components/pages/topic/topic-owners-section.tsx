...
interface TopicOwnersSectionProps {
  topic: string;
  serverTopOwners: TopOwner[];
  serverTopOwnersCount: number;
}

export function TopicOwnersSection({
  topic,
  serverTopOwners,
  serverTopOwnersCount,
}: TopicOwnersSectionProps) {
  const clientData = useQuery(api.queries.stack.getTopicPageData, {
    topic,
  });
  ...
  return (
    <>
      ...
      {hasBlurredItems && gatedCount > 0 && (
        <SignInGateCta
          message={`Sign in to see all ${topOwnersCount} developers using #${topic}`}
          className="mt-6"
        />
      )}
    </>

...
import { ProfileClaimWatchButton } from "@/components/social/profile-claim-watch/profile-claim-watch-button";
...
interface OptimisticStarsState {
  baseStarsReceived: number | null;
  delta: number;
}

function applyOptimisticStarDelta(
  current: OptimisticStarsState,
  baseStarsReceived: number,
  delta: number
): OptimisticStarsState {
  const currentDelta = current.baseStarsReceived === baseStarsReceived ? current.delta : 0;

  return {
    baseStarsReceived,
    delta: Math.max(-baseStarsReceived, currentDelta + delta),
  };
}
...
interface ProfileAvatarScoreProps {
  owner: string;
  profileImage: string;
  stackScore: number;
  isOnline: boolean;
        ...
        content={
          <div className="max-w-56 space-y-2 text-left">
            ...
            <p className="text-xs font-medium leading-relaxed text-muted-foreground dark:text-zinc-300">
              A profile strength signal based on identity, stack depth, profile context, invites,
              and community trust. Higher scores unlock stronger discovery, follows, and messaging.
            </p>
          </div>
        }
      />
...
interface ProfileTitleDetailsProps {
  owner: string;
  profile: ProfileHeaderProps["profile"];
  followCounts: ProfileHeaderProps["followCounts"];
  state: {
    isHydrating: boolean;
    isOrganization: boolean;
...
function ProfileTitleDetails({ owner, profile, followCounts, state }: ProfileTitleDetailsProps) {
  const { isHydrating, isOrganization, isOrganizationVerified, isOwnerViewer, isOwnershipPending } =
    state;
...
function ProfileIdentity(props: ProfileIdentityProps) {
  return (
    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 lg:gap-4 flex-1 min-w-0">
      <ProfileAvatarScore
        owner={props.owner}
        profileImage={props.profileImage}
        stackScore={props.stackScore}
        isOnline={props.state.isOnline}
        isOwnerViewer={props.state.isOwnerViewer}
        isOwnershipPending={props.state.isOwnershipPending}
      />
      <ProfileTitleDetails
        owner={props.owner}
...
interface VisitorProfileActionsProps {
  owner: string;
  shareUrl: string;
  ...
  isOrganization: boolean;
  isStarredByViewer: boolean;
  optimisticStarsReceived: number;
  viewerStackScore: number;
  onStarDelta: (delta: number) => void;
}

function VisitorProfileActions({
  owner,
  shareUrl,
  profileShareText,
  isClaimed,
    ...
    <div className="flex w-full flex-col gap-2 sm:ml-auto sm:max-w-md lg:w-72 lg:max-w-none">
      ...
      >
        <StarButton
          ...
          variant="action"
          onStarDelta={onStarDelta}
        />
        {isClaimed && !isOrganization && (
          <FollowButton targetOwner={owner} viewerStackScore={viewerStackScore} />
        )}
        {!isClaimed && !isOrganization && <ProfileClaimWatchButton targetOwner={owner} />}
      </div>
...
interface OwnerProfileActionsProps {
  ...
  stackScore: number;
  checklistState: {
    hasBioAndSocials: boolean;
    hasPersonalizedPrivateStack: boolean;
    hasStackDepth: boolean;
  };
  referralPoints: number;
  communityScore: number;
}

function OwnerProfileActions({
  owner,
  shareUrl,
  profileShareText,
  profileState,
  ...
  referralPoints,
  communityScore,
}: OwnerProfileActionsProps) {
  const { isClaimed, isOrganization } = profileState;
  const { hasBioAndSocials, hasPersonalizedPrivateStack, hasStackDepth } = checklistState;
          ...
          >
            <div className="px-1 pb-3">
              ...
              <div className="mt-2 flex items-end justify-between gap-3">
                <p className="text-sm font-bold text-foreground dark:text-white">Current score</p>
            ...
            <ul className="space-y-2">
              ...
              />
              <ScoreGrowthItem
                done={hasPersonalizedPrivateStack}
...
interface ProfileHeaderActionsProps extends VisitorProfileActionsProps {
  hasBioAndSocials: boolean;
  hasPersonalizedPrivateStack: boolean;
  hasStackDepth: boolean;
  isHydrating: boolean;
...
function ProfileHeaderActions({
  ...
  if (isOwnershipPending) {
    return <PendingProfileActions />;
  }

  if (!isHydrating && !isOwnerViewer) {
    return <VisitorProfileActions {...props} />;
  }
  ...
  return (
    <OwnerProfileActions
      ...
      profileShareText={props.profileShareText}
      profileState={{
      ...
      stackScore={props.stackScore}
      checklistState={{
        hasBioAndSocials: props.hasBioAndSocials,
        hasPersonalizedPrivateStack: props.hasPersonalizedPrivateStack,
        hasStackDepth: props.hasStackDepth,
      }}
      referralPoints={props.referralPoints}
      communityScore={props.communityScore}
    />
  );
}
...
export function ProfileHeader({
  ...
  const [optimisticStarsState, setOptimisticStarsState] = useState<OptimisticStarsState>({
    baseStarsReceived: null,
    delta: 0,
  });
  const optimisticStarDelta =
    optimisticStarsState.baseStarsReceived === starsReceived ? optimisticStarsState.delta : 0;
  const optimisticStarsReceived = Math.max(0, starsReceived + optimisticStarDelta);
      ...
      <div className="relative z-10 flex flex-col gap-6 p-5 sm:p-8 lg:flex-row lg:gap-8">
        ...
        <div className="flex w-full shrink-0 flex-col self-stretch lg:w-auto lg:self-start">
          <ProfileHeaderActions
            ...
            optimisticStarsReceived={optimisticStarsReceived}
            viewerStackScore={viewerStackScore}
            isHydrating={isHydrating}
            isOwnerViewer={isOwnerViewer}
            isOwnershipPending={isOwnershipPending}
            stackScore={stackScore}
            hasBioAndSocials={hasBioAndSocials}
            hasPersonalizedPrivateStack={summary.personalizedWithPrivate}
            hasStackDepth={hasStackDepth}
            referralPoints={referralPoints}
            communityScore={communityScore}
            onStarDelta={(delta) => {
              setOptimisticStarsState((current) =>
                applyOptimisticStarDelta(current, starsReceived, delta)
              );
            }}
          />
        </div>
